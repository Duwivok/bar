...
let prepNameSet = new Set(); // имена заготовок (recipes.is_prep=true) — для защиты от совпадения имён с сырьём
...
let packagePriceChecks = {}; // packageId/tempId -> { state, result }

const PRICE_SOURCE_FIELD_KEYS = [
    "price_source_type",
    "price_source_query",
    "price_source_enabled",
];
const OPTIONAL_PACKAGE_FIELD_KEYS = [
    "purchase_source",
    ...PRICE_SOURCE_FIELD_KEYS,
];
...
function packageFieldLabel(fieldKey, baseUnit) {
    const labels = {
        package_size: baseUnit ? `Размер (${baseUnit})` : "Размер",
        package_price: "Цена",
        purchase_unit: "Закупочная ед.",
        purchase_source: "Источник",
        purchase_link: "Ссылка",
        price_source_type: "Парсер",
        price_source_query: "Запрос",
        price_source_enabled: "Автоцена",
    };
...
function buildPackageRow(ingredientId, pkg, locked, baseUnit) {
    ...
    const packageKey = pkg.id || pkg.tempId;
    ...
    PACKAGE_FIELDS.forEach((field) => {
        ...
        fieldWrap.className = "package-field package-field-" + field.key;
        const labelText = document.createElement("span");
        labelText.className = "package-field-label";
        labelText.textContent = packageFieldLabel(field.key, baseUnit);
    ...
    actionsWrap.className = "package-actions";
    if (pkg.id) {
        const checkBtn = document.createElement("button");
        checkBtn.type = "button";
        checkBtn.textContent = "Проверить";
        checkBtn.onclick = () => checkPackagePrice(ingredientId, { ...pkg, ...readValues(inputs, PACKAGE_FIELDS) }, packageKey);
    ...
    const inlineCheck = buildPackagePriceCheck(packageKey);
    if (inlineCheck) card.appendChild(inlineCheck);
    return card;
}
...
function buildPackagesBlock(record, locked) {
    ...
    if (pkgs.length === 0) {
        ...
        empty.textContent = "Пока ни одного варианта — добавьте хотя бы один, чтобы позиция считалась заполненной.";
        list.appendChild(empty);
    } else {
        pkgs.forEach((pkg) => list.appendChild(buildPackageRow(record.id, pkg, locked, record.base_unit)));
    }
...
async function savePackageRow(ingredientId, pkg, inputs, opts = {}) {
    ...
    if (error && isOptionalPackageSchemaError(error)) {
        const legacyValues = withoutOptionalPackageFields(values);
        if (pkg.id) {
            ({ error } = await db.from("ingredient_packages").update(legacyValues).eq("id", pkg.id));
        } else {
            ({ error } = await db.from("ingredient_packages").insert({ ...legacyValues, ingredient_id: ingredientId }));
            if (!error) packageDrafts[ingredientId] = (packageDrafts[ingredientId] || []).filter((p) => p.tempId !== pkg.tempId);
        }
        if (!error) {
            showToast("Упаковка сохранена частично: нужно накатить schema_purchase_source.sql и schema_price_sources.sql", "error");
        }
    }
    if (error) {
...
function buildRow(record, kind) {
    ...
    if (record.id) {
        const checkPricesBtn = document.createElement("button");
        checkPricesBtn.type = "button";
        checkPricesBtn.textContent = "Проверить цены";
        checkPricesBtn.title = "Проверить цены всех упаковок этой позиции, не раскрывая строку";
        checkPricesBtn.onclick = () => checkIngredientPrices(record.id, rowKey);
        actions.appendChild(checkPricesBtn);
    }
...
async function saveRow(record, inputs) {
    ...
    if (!confirmPrepNameCollision(values.name)) return;
    flashInputs(inputs);
    const rowKey = record.id || record.tempId;
    let error;
    if (record.id) {
...
async function checkPackagePrice(ingredientId, pkg, packageKey) {
    ...
    try {
        ...
        packagePriceChecks[packageKey] = {
            state: "error",
            result: { message: error.message || "Не удалось проверить цену", status: "error", packageId: packageKey },
        };
    }
    render();
}
...
async function checkIngredientPrices(ingredientId, rowKey) {
    const items = collectIngredientPriceItems(ingredientId);
    if (items.length === 0) {
        showToast("Нет сохранённых вариантов упаковки с включённой автоценой", "error");
        return;
    }
    ...
    try {
        ...
        items.forEach((item) => {
            packagePriceChecks[item.packageId] = {
                state: "error",
                result: { message: error.message || "Не удалось проверить цену", status: "error", packageId: item.packageId },
            };
        });
    }
    render();
...
async function applyPriceResult(result) {
    ...
    Object.keys(packagePriceChecks).forEach((key) => {
        ...
        if (state && state.result && state.result.packageId === result.packageId) {
            packagePriceChecks[key] = { ...state, result: { ...state.result, applied: true, oldPrice: result.newPrice } };
        }
    });
...
function buildPackagePriceCheck(packageKey) {
    ...
    if (state.state === "loading") {
        wrap.textContent = "Проверяю цену...";
        return wrap;
    }

    const result = state.result;
    if (!result) {
        wrap.textContent = "Сервис не вернул результат.";
        return wrap;
    }

    wrap.appendChild(buildPriceResultCard(result, () => dismissPackagePriceResult(packageKey)));
    return wrap;
}
...
function renderPriceCheckPanel() {
    ...
    priceCheckResults.forEach((result) => {
        priceCheckPanel.appendChild(buildPriceResultCard(result, () => dismissPriceResult(result)));
    });
}
...
function render() {
    // Пересборка секций временно опустошает таблицу, из-за чего браузер обрезает
    // scroll-позицию до нуля — запоминаем её и восстанавливаем после перерисовки,
    // иначе любое действие внутри развёрнутой строки (например "Проверить цену")
    // подбрасывает страницу наверх.
...
async function loadIngredients() {
    const [ingRes, pkgRes, convRes, prepRes] = await Promise.all([
...
// Защита от дурака: имя сырья совпадает с уже существующей заготовкой в Рецептах. Такое сырьё
// станет "невидимым двойником" заготовки — рецепты будут путаться, к чему из двух привязываться
// (см. историю с "Цедра лимона": заготовку и сырьё с одинаковым именем нельзя различить по названию).
// Не блокирует сохранение — только явно спрашивает, точно ли это нужно.
...
document.getElementById("bulkImportBtn").onclick = async () => {
    ...
    if (collisions.length > 0) {
        const proceed = confirm(
            `Уже существуют как заготовки в Рецептах: ${collisions.map((n) => `«${n}»`).join(", ")}. ` +
            `Если это то же самое — используйте заготовку, а не заводите одноимённое сырьё, иначе они будут путаться. Всё равно продолжить импорт?`
        );
        if (!proceed) return;
    }
...
async function importExcelFile(file) {
    ...
    if (newCollisions.length > 0) {
        const proceed = confirm(
            `Уже существуют как заготовки в Рецептах: ${newCollisions.map((n) => `«${n}»`).join(", ")}. ` +
            `Если это то же самое — используйте заготовку, а не заводите одноимённое сырьё, иначе они будут путаться. Всё равно продолжить импорт?`
        );
        if (!proceed) return;
    }
